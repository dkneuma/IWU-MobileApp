...
app.get('/chapel', function(req, res){
  res.header("Access-Control-Allow-Origin", "*");
  res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
  res.sendFile(path.join(__dirname, 'XML/chapel.json'));
});
...
app.get('/votd', function(req, res){
  res.header("Access-Control-Allow-Origin", "*");
  res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
  res.sendFile(path.join(__dirname, 'XML/votd.json'));
...
//Places the array of items into an rss feed, then puts the feed into news.xml
  ...
  newsString = newsString.replace(/(\\t0|\\n|\\t|]]>|\uFFFD)/g,"");
  newsString = newsString.replace(/â€™/g, "");
  //newsString = newsString.replace(/\&/g,"&amp;");
